...
int main(int argc, char **argv){
    ...
    printf("[V3] Usant %d fils\n", omp_get_max_threads());
    printf("[V3] Buscant fins a %u iteracions...\n", maxseed);
    ...
    #pragma omp parallel shared(trobat, seed_trobat, pwd_trobat, target, maxseed, pwdlen)
    {
        ...
        // Reduïm checks de trobat al mínim absolut
        uint32_t local_check_counter = 0;
        const uint32_t CHECK_EVERY = 65536; // Cada 64K iteracions
        int local_trobat = 0;

        // schedule(static) per mínim overhead en rangs grans
        // Cada thread agafa un bloc gran i el processa
        #pragma omp for schedule(static) nowait
        for(uint32_t i = 0; i < maxseed; i++){
            // Check ultra-esporàdic de trobat
            if(__builtin_expect(++local_check_counter >= CHECK_EVERY, 0)){
            ...
            // Si ja s'ha trobat, skip
            if(local_trobat) continue;
    ...
    if(!trobat){
        printf("\n[V3] Sense coincidències en %u intents.\n", maxseed);
        printf("  temps (s)   = %.6f\n", ttotal);
